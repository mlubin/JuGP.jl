...
#     @defVar(m, s[k=1:M])
#
#     @setNLObjective(m, Min, prod{t[k], k=1:q} * prod{s[k], k=1:M})
#
#     @addNLConstraint(m, lt[k=1:q], (z[1]^X[k,1]) * (z[2]^X[k,2]) <= t[k])
#     @addNLConstraint(m, ls[k=1:M], 1 + (z[1]^(-X[k,1])) * (z[2]^(-X[k,2])) <= s[k])
#
#     # test continuous problem
#     status = solve(m)
#     @test status == :Optimal
#     # comparing optimal continuous answers with cvx solutions
#     @test_approx_eq_eps getObjectiveValue(m) 2.150919077978715e4 1e-3
#     @test_approx_eq_eps log(getValue(z[1])) 0.499270692481327 1e-4
...
#     @defVar(m, y[i=1:N+1])
#
#     @setNLObjective(m, Min, sum{w[i] * h[i], i=1:N})
#
#     @defNLExpr(m, d[i=1:N], 6*F / (E * w[i] * h[i]^3))
#     @addNLConstraint(m, vle[i=1:N], (2*i-1) * d[i] + v[i+1] <= v[i])
#     @addNLConstraint(m, yle[i=1:N], (i-1/3) * d[i] + v[i+1] + y[i+1] <= y[i])
#
#     @addNLConstraint(m, sMax[i=1:N], h[i] / w[i] <= Smax)
#     @addNLConstraint(m, sMin[i=1:N], h[i] / w[i] >= Smin)
#     @addNLConstraint(m, sig[i=1:N], 6*i*F / (w[i] * h[i]^2) <= sigma_max)
#     @addNLConstraint(m, yMax, y[1] <= ymax)
#
#     # test continuous problem
#     status = solve(m)
#     @test status == :Optimal
#     # comparing optimal answers with cvx solution
#     @test_approx_eq_eps getObjectiveValue(m) 42.396550031186266 1e-5
...
#     @test_approx_eq_eps getValue(w) w_opt 2e-5
